...
class AllJobs extends Component {
  ...
  onGetJobDetails = async () => {
    this.setState({apiJobsStatusConstants: apiJobsStatusConstants.inProgress})
    const jwtToken = Cookies.get('jwt_token')
    const {checkboxInputs, radioInput, searchInput} = this.state
    const jobsApiUrl = `https://apis.ccbp.in/jobs?employment_type=${checkboxInputs}&mininum_package=${radioInput}&search=${searchInput}`
    const optionsJobs = {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
  ...
  onGetInputOption = event => {
    ...
    const inputNotInList = checkboxInputs.filter(
      eachItem => eachItem === event.targetid,
    )
